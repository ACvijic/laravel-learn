...
@section('seo-title')
<title> Edit menu - {{ $menu->title }} {{ config('app.seo-title-separator') }} {{ config('app.name') }}</title>
@endsection
...
<div class="container-fluid">
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">Edit menu: {{ $menu->title }}</h1>
        </div>
        <!-- /.col-lg-12 -->
    </div>
    <!-- /.row -->
                ...
                <div class="panel panel-default">
                    <div class="panel-body">
                        ...
                        <div class="form-group{{ $errors->has('parent_id') ? ' has-error' : '' }}">
                            ...
                            <select class="form-control" name="parent_id">
                                ...
                                @if(count($menusFirstLevel) > 0)
                                    ...
                                    <option value="{{ $menu->id }}" @if(old('parent_id', $menu->parent_id) == $value->id) selected @endif>{{ $value->title }}</option>
                                    @if(count($value->submenus) > 0)
                                        @foreach($value->submenus as $submenu)
                                        <option style="text-indent: 30px;" value="{{ $submenu->id }}" @if(old('parent_id', $menu->parent_id) == $submenu->id) selected @endif>{{ $submenu->title }}</option>
                                        @endforeach
                                    @endif
                                    @endforeach
                                @endif
                        ...
                        <div class="form-group{{ ($errors->has('position') || $errors->has('position.*')) ? ' has-error' : '' }}">
                            ...
                            <div class="checkbox">
                                <label>
                                    <input name="position[]" type="checkbox" value="top" @if(!empty(old('position', $positionsOld)) && in_array('top', old('position', $positionsOld))) checked @endif>Top menu
                                </label>
                            </div>
                            <div class="checkbox">
                                <label>
                                    <input name="position[]" type="checkbox" value="header" @if(!empty(old('position', $positionsOld)) && in_array('header', old('position', $positionsOld))) checked @endif>Header
                                </label>
                            </div>
                            <div class="checkbox">
                                <label>
                                    <input name="position[]" type="checkbox" value="sidebar" @if(!empty(old('position', $positionsOld)) && in_array('sidebar', old('position', $positionsOld))) checked @endif>Sidebar
                                </label>
                            </div>
                            <div class="checkbox">
                                <label>
                                    <input name="position[]" type="checkbox" value="footer" @if(!empty(old('position', $positionsOld)) && in_array('footer', old('position', $positionsOld))) checked @endif>Footer
                                </label>
                            </div>
